...
async function writeExifVid(media, metadata) {
  ...
  let packname 
  let author 
  try { packname = metadata.packname} catch (error) {  packname = "SIGMA_MD"}
  try {  author = metadata.author} catch (error) {  author = ""}
    const img = new webp.Image();
    const json = {
      "sticker-pack-id": `SIGMA-MD`,
...
async function writeExifWebp(media, metadata) {
  ...
  if (metadata.packname || metadata.author) {
    const img = new webp.Image();
    const json = {
      "sticker-pack-id": `SIGMA_MD-MAHER-ZUBAIR`,
      "sticker-pack-name": metadata.packname,
      "sticker-pack-publisher": metadata.author,
      emojis: metadata.categories ? metadata.categories : [""],
    };
